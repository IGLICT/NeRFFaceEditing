...
def setup_snapshot_image_grid(training_set, random_seed=0):
    ...
    gw = 8 # np.clip(7680 // training_set.image_shape[2], 7, 32)
...
@torch.no_grad()
def vis_parsing_maps(im: torch.Tensor, argmax: bool = True):
    part_colors = [
        [0, 0, 0], # Background
        [127, 212, 255], # Skin
...
def save_image_grid(img, fname, drange, grid_size, mode='img'):
    if mode == 'seg':
        img = vis_parsing_maps(img).numpy()
    lo, hi = drange
...
def training_loop(
    ...
    if (resume_pkl is not None) and (rank == 0):
        ...
        for name, module in [('G', G), ('G_ema', G_ema)]:
            with torch.no_grad():
                module.decoder.geo_net[0].load_state_dict(resume_data[name].decoder.net[0].state_dict())
                module.decoder.geo_net[2].weight[:1, :] = resume_data[name].decoder.net[2].weight.data[:1, :].clone()
                module.decoder.geo_net[2].bias[:1] = resume_data[name].decoder.net[2].bias.data[:1].clone()
                assert isinstance(module.decoder.geo_net[2].weight, torch.nn.Parameter)
                assert isinstance(module.decoder.geo_net[2].bias, torch.nn.Parameter)
                
                module.decoder.app_net[0].load_state_dict(resume_data[name].decoder.net[0].state_dict())
                module.decoder.app_net[2].weight = torch.nn.Parameter(resume_data[name].decoder.net[2].weight.data[1:, :].clone())
                module.decoder.app_net[2].bias = torch.nn.Parameter(resume_data[name].decoder.net[2].bias.data[1:].clone())
                
                module.decoder.train().requires_grad_(False).to(device)
    ...
    if rank == 0:
        ...
        # Split by row
        grid_z = torch.randn([labels.shape[0], G.z_dim], device=device).split(grid_size[1])
        grid_c = torch.from_numpy(labels).to(device).split(grid_size[1])
    ...
    while True:
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            out = [G_ema(z=z, c=c, noise_mode='const', planes_mean=0, planes_var=0) for z, c in zip(grid_z, grid_c)]
            ...
            images_seg = torch.cat([o['image_seg'].cpu() for o in out])
            ...
            save_image_grid(images_seg, os.path.join(run_dir, f'fakes{cur_nimg//1000:06d}_seg.png'), drange=[-1,1], grid_size=grid_size, mode='seg')
