...
class SegMipRayMarcher2(nn.Module):
    def __init__(self):
        super().__init__()
    def run_forward(self, colors, segs, densities, depths, rendering_options):
        deltas = depths[:, :, 1:] - depths[:, :, :-1]
        colors_mid = (colors[:, :, :-1] + colors[:, :, 1:]) / 2
        segs_mid = (segs[:, :, :-1] + segs[:, :, 1:]) / 2
        densities_mid = (densities[:, :, :-1] + densities[:, :, 1:]) / 2
        depths_mid = (depths[:, :, :-1] + depths[:, :, 1:]) / 2
        ...
        composite_rgb = torch.sum(weights * colors_mid, -2)
        composite_seg = torch.sum(weights * segs_mid, -2)
        weight_total = weights.sum(2)
        composite_depth = torch.sum(weights * depths_mid, -2) / weight_total
        ...
        return composite_rgb, composite_seg, composite_depth, weights


    def forward(self, colors, segs, densities, depths, rendering_options):
        composite_rgb, composite_seg, composite_depth, weights = self.run_forward(colors, segs, densities, depths, rendering_options)
